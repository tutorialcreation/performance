...
def dashboard(request):
    ...
    dataset['teams'] = Team.objects.all()
...
def dashboard_employees_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            nationality_id = request.POST.get('nationality', None)
...
def dashboard_employee_info(request, id):
    ...
    employee = get_object_or_404(Employee, id=id)
    employee_emergency_instance = Emergency.objects.filter(
        employee=employee).first()
    employee_family_instance = Relationship.objects.filter(
        employee=employee).first()
...
def dashboard_emergency_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Emergency Successfully Created for {0}'.format(
                emp_name), extra_tags='alert alert-success alert-dismissible show')
            return redirect('dashboard:emergencycreate')

        else:
            messages.error(request, 'Error Creating Emergency for {0}'.format(
                emp_name), extra_tags='alert alert-warning alert-dismissible show')
            return redirect('dashboard:emergencycreate')
...
def dashboard_emergency_update(request, id):
    ...
    dataset['form'] = form
    dataset['title'] = 'Updating Emergency Details for {0}'.format(
...
def dashboard_family_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Relationship Successfully Created for {0}'.format(
                employee_object), extra_tags='alert alert-success alert-dismissible show')
            return redirect('dashboard:familycreate')
        else:
            messages.error(request, 'Failed to create Relationship for {0}'.format(
                employee_object), extra_tags='alert alert-warning alert-dismissible show')
            return redirect('dashboard:familycreate')
...
def dashboard_family_edit(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Relationship Successfully Updated for {0}'.format(
                employee.get_full_name), extra_tags='alert alert-success alert-dismissible show')
            return redirect('dashboard:familycreate')

        else:
            messages.error(request, 'Failed to update Relationship for {0}'.format(
                employee.get_full_name), extra_tags='alert alert-warning alert-dismissible show')
            return redirect('dashboard:familycreate')
...
def dashboard_bank_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Account Successfully Created for {0}'.format(
                employee_object.get_full_name), extra_tags='alert alert-success alert-dismissible show')
            return redirect('dashboard:bankaccountcreate')
        else:
            messages.error(request, 'Error Creating Account for {0}'.format(
                employee_object.get_full_name), extra_tags='alert alert-warning alert-dismissible show')
            return redirect('dashboard:bankaccountcreate')
...
def employee_bank_account_update(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Account Successfully Edited for {0}'.format(
                employee.get_full_name), extra_tags='alert alert-success alert-dismissible show')
            return redirect('dashboard:bankaccountcreate')
        else:
            messages.error(request, 'Error Updating Account for {0}'.format(
                employee.get_full_name), extra_tags='alert alert-warning alert-dismissible show')
            return redirect('dashboard:bankaccountcreate')
    ...
    form = BankAccountCreation(
        request.POST or None, instance=bank_instance_obj)
...
def approve_leave(request, id):
    ...
    messages.error(request, 'Leave successfully approved for {0}'.format(
        employee), extra_tags='alert alert-success alert-dismissible show')
...
def birthday_this_month(request):
    ...
    # am using this to get the month for template rendering- making it dynamic
