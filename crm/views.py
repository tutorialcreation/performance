...
def filter_parameter(request):
    client_list = ClientManager.objects.all()
    client_filter = ClientFilter(request.GET, queryset=client_list)
    contact_list=ContactManager.objects.all()
    contact_filter=ContactFilter(request.GET,queryset=contact_list)
    client_name=request.GET.get('client_name')
    client_country=request.GET.get('country')
    client_tel_no=request.GET.get('client_tel_no')
    ...
    nature_of_assignment=request.GET.get('nature_of_assignment')
    contract_period=request.GET.get('contract_period')
    client_query=[client_name,client_country,client_tel_no,client_email,
                client_location,client_status,nature_of_assignment,contract_period]
    # print(client_query)
    # print(res)
    client_results=ClientManager.objects.filter(
                            Q(country__in=client_query)|
                            Q(client_status__in=client_query)| 
                            Q(client_name__in=client_query)| 
                            Q(client_tel_no__in=client_query)|
                            Q(client_location__in=client_query)|
                            Q(client_email__in=client_query)
    )
    client_results=[client.client_name for client in client_results]

    contact_name=request.GET.get('contact_name')
    contact_position=request.GET.get('position')
    contact_tel_no=request.GET.get('personal_tel_no')
    contact_email=request.GET.get('personal_email')
    contact_status=request.GET.get('personal_status')
    contact_client=request.GET.get('client')
    contact_service_line=request.GET.get('service_line')
    contact_query=[contact_name,contact_position,contact_tel_no,contact_email,
                contact_status, contact_client,contact_service_line]
    contact_results=ContactManager.objects.filter(
                            Q(contact_name__in=contact_query)|
                            Q(position__in=contact_query)|
                            Q(personal_status__in=contact_query)| 
                            Q(personal_tel_no__in=contact_query)|
                            Q(personal_email__in=contact_query) |
                            Q(service_line__in=contact_query)
                )
    contact_results=[contact.contact_name for contact in contact_results]
    contact_query=list(filter(None,contact_query))
    client_query=list(filter(None,client_query))
    search_query=[contact_query,client_query]
    # import pdb;pdb.set_trace()
    # search_query=",".join(search_query)
    # print(contact_query)
    if contact_results or client_results:
        FilterSet.objects.create(search_parameter=search_query,
        filtered_list=[contact_results,client_results],user_filters=request.user)
    # print(contacts)            
    ...
    return render(request, 'crm/communications.html', {
                        'form': form,
                        'filters':True,
...
def client_list(request):
    context={}
    context['clients']=ClientManager.objects.all()
    return render(request,'crm/clients.html',context)

def contact_list(request):
    context={}
    context['contacts']=ContactManager.objects.all()
